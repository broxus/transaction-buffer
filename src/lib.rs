...
pub fn start_parsing_and_get_channels(config: BufferedConsumerConfig) -> BufferedConsumerChannels {
    ...
    let pipeline_span = tracing::info_span!("transaction_buffer_pipeline");
    #[cfg(not(feature = "compact-logs"))]
    let pipeline_span = tracing::info_span!(
        "transaction_buffer_pipeline",
        rocksdb_path = %config.rocksdb_path,
        buff_size = config.buff_size,
        commit_time_secs = config.commit_time_secs,
...
#[cfg_attr(
    not(feature = "compact-logs"),
    tracing::instrument(
        level = "info",
        skip(tx_parsed_events, notify_for_services, commit_rx, rocksdb_client, config),
        fields(
            rocksdb_path = %config.rocksdb_path,
            buff_size = config.buff_size,
            commit_time_secs = config.commit_time_secs,
...
async fn parse_kafka_transactions(
...

    {
        ...
        tokio::spawn(commit_transactions(commit_rx, context).instrument(span));
    }
    ...
    while let Some(produced_transaction) = stream_transactions.next().await {
        ...
        *context.timestamp_last_block.write().await = transaction_time as i32;
            ...
            || *context.time.read().await >= context.config.commit_time_secs
        {
            ...
            tracing::info!(
                count,
                timestamp_block = transaction_time,
                date = %DateTime::from_timestamp(transaction_time.into(), 0)
                    .unwrap()
                    .date_naive(),
                "sync kafka commit"
            );
...
#[cfg_attr(
    feature = "compact-logs",
    tracing::instrument(level = "info", skip(context, offsets))
)]
#[cfg_attr(
    not(feature = "compact-logs"),
    tracing::instrument(
        level = "info",
        skip(context, offsets),
        fields(
            transactions_logger_counter = context.transactions_logger_counter,
        )
    )
)]
    ...
    while let Some(produced_transaction) = stream_transactions.next().await {
        ...
        *context.time.write().await = 0;
        ...
        produced_transaction
            .message
            .store_offset(&context.config.transaction_consumer.topic)
            .expect("dead stream kafka");

        if i >= context.transactions_logger_counter {
            tracing::info!(
                count = context.transactions_logger_counter,
                timestamp_block = transaction_timestamp,
                date = %DateTime::from_timestamp(transaction_timestamp as i64, 0).unwrap(),
                "realtime kafka heartbeat"
            );
            i = 0;
        }
    }
...
#[cfg_attr(
    feature = "compact-logs",
    tracing::instrument(level = "info", skip(tx, context))
)]
#[cfg_attr(
    not(feature = "compact-logs"),
    tracing::instrument(
        ...
        skip(tx, context),
        fields(
            first_iterate_delay = context.first_iterate_delay.unwrap_or(60),
            cache_timer = context.config.cache_timer,
        )
    )
)]
async fn parse_transaction(
    ...
    while count_not_processed >= 10_000 || is_first_iterate {
        if is_first_iterate {
            tracing::info!(count_not_processed, "start first local base iterate");
            ...
            tracing::info!(count_not_processed, "start next local base iterate");
        }
        ...
        for transaction in transactions_iter {
            ...
            if i % 10_000 == 0 {
                tracing::info!(current = i, total = count_not_processed, "parsing progress");
            }
        }
    ...
    tracing::info!("services notified");
...
#[tracing::instrument(level = "debug", skip(commit_rx, context))]
...
mod test {
    ...
    async fn test_count_last_key_logic() {
        ...
        let _index = Utc::now().timestamp() as u32;
